...
CREATE TABLE `warehouses` (
  ...
  `name` VARCHAR(50)  NOT NULL COMMENT 'Nombre del almacén',
  `comment` VARCHAR(100) NOT NULL DEFAULT '' COMMENT 'Comentario adicional del usuario',
  `width` TINYINT(3) UNSIGNED NOT NULL COMMENT 'Ancho en celdas del almacén',
  `height` TINYINT(3) UNSIGNED DEFAULT NULL,
  `uuid_user_creation` VARCHAR(36)  NOT NULL COMMENT 'uuid del usuairo que la creo',
  `datehour_creation` DATETIME NOT NULL COMMENT 'Fecha y hora de creación',
  `uuid_user_modification` VARCHAR(36)  DEFAULT NULL,
  `datehour_modification` DATETIME DEFAULT NULL COMMENT 'fecha y hora de la ultima modificación',
  PRIMARY KEY (`id`),
...
CREATE TABLE `scales` (
  `id` INT(11) UNSIGNED NOT NULL AUTO_INCREMENT,
  `uuid` VARCHAR(36) NOT NULL COMMENT 'uuid',
  `end_device_id` VARCHAR(50) NOT NULL COMMENT 'id del dispositivo en TTN',
  `voltage_min` DECIMAL(5,3) UNSIGNED NOT NULL  DEFAULT '3.2'COMMENT 'Voltage mínimo',
