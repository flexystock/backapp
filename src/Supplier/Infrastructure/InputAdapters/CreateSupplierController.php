...
class CreateSupplierController extends AbstractController
{
    ...
    private CreateSupplierUseCaseInterface $createSupplierUseCase;
    private LoggerInterface $logger;

    public function __construct(
        CreateSupplierUseCaseInterface $createSupplierUseCase,
    ...
    #[Route('/api/supplier', name: 'api_create_supplier', methods: ['POST'])]
    #[RequiresPermission('supplier.create')]
    #[OA\Post(
        path: '/api/supplier',
        summary: 'Crear un nuevo proveedor global en la BBDD main',
    ...
    public function __invoke(Request $request): JsonResponse
    {
        try {
            ...
            $useCaseRequest = new CreateSupplierRequest(
                $data['name'],
                $data['slug'],
                $data['logoUrl'] ?? null,
                $data['website'] ?? null,
            ...
            $response = $this->createSupplierUseCase->execute($useCaseRequest);
            ...
            $this->logger->error('Error in CreateSupplierController', [
                'error' => $e->getMessage(),
